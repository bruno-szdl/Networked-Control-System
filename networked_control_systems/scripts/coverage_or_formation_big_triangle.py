...
class SubAndPub:
    def __init__(self):
        ...
        edge = 1
        self.distance_between_robots = {"tb3_0" : np.array([[edge, edge, edge*2, 0, edge*2], [edge*2, edge*2, edge*4, edge*4, edge*4]]),
                                        "tb3_1" : np.array([[edge, edge*2, edge, edge, edge*3], [edge*2, 0, edge*2, edge*2, edge*2]]),
                                        "tb3_2" : np.array([[edge, edge*2, edge*3, edge, edge], [edge*2, 0, edge*2, edge*2, edge*2]]),
                                        "tb3_3" : np.array([[edge*2, edge, edge*3, edge*2, edge*4], [edge*4, edge*2, edge*2, 0, 0]]),
                                        "tb3_4" : np.array([[0, edge, edge, edge*2, edge*2], [edge*4, edge*2, edge*2, 0, 0]]),
                                        "tb3_5" : np.array([[edge*2, edge*3, edge, edge*4, edge*2], [edge*4, edge*2, edge*2, 0, 0]])}
    ...
    def get_distances(self, this_robot_pos, other_robots_pos):
        ...
        desired_distances = self.distance_between_robots[self.name]
        ...
        for i in range(self.n_robots-1):
            if (distances[2,i] < self.min_dist) and (random.random() < self.chances):
                    n += 1
                    sum_x += self.gain*(abs(distances[0,i])**2 - desired_distances[0,i]**2)*distances[0,i]
                    sum_y += self.gain*(abs(distances[1,i])**2 - desired_distances[1,i]**2)*distances[1,i]
